...
// the create method also creates the update() method when it's done 
d3OhlcChart.create = function(bindTo, props, state){
  // initialize
  var margin = {top: 20, right: 20, bottom: 30, left: 50},
    width = $(bindTo).width() - margin.left - margin.right,
    height = $(bindTo).height() - margin.top - margin.bottom;
  ...
  // kick off with data from state
  var data, feed;
  var accessor = ohlc.accessor();
  feed = state.data.map(function(d) {
    return {
  ...
  //////////////////////////////////////////////////////////
  // define the update() method all the local chart stuff.
  //////////////////////////////////////////////////////////
  d3OhlcChart.update = function(data) {
    ...
    d3OhlcChart.refreshIndicator(svg.select("g.sma.ma-0"), sma0, sma0Calculator(data));
    d3OhlcChart.refreshIndicator(svg.select("g.sma.ma-1"), sma1, sma1Calculator(data));
...
d3OhlcChart.destroy = function() {
};

d3OhlcChart.refreshIndicator = function(selection, indicator, data) {
